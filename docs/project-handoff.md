...
### 2026-09-12 - Public release review and corrective work

- A project-wide review was performed against every repository's authoritative `origin/main` before public release. The review covered financial/event integrity, authorization, secrets, Kubernetes/Helm, CI, API contracts, and public documentation.
- The first corrective wave is merged: fail-closed API Gateway defaults ([api-gateway #28](https://github.com/digital-bank-java/api-gateway/pull/28)), Auth fixture profile gating ([auth-service #14](https://github.com/digital-bank-java/auth-service/pull/14)), ledger-failure reservation release ([transaction-service #33](https://github.com/digital-bank-java/transaction-service/pull/33)), ledger outcome identity validation ([account-service #46](https://github.com/digital-bank-java/account-service/pull/46)), transfer-created payload validation ([notification-service #15](https://github.com/digital-bank-java/notification-service/pull/15)), duplicate SIT configuration removal ([config-repo #52](https://github.com/digital-bank-java/config-repo/pull/52)), and SIT topic/validation alignment ([infra-sit #45](https://github.com/digital-bank-java/infra-sit/pull/45)).
- The organization profile README and review evidence are in [.github #270](https://github.com/digital-bank-java/.github/pull/270). The profile describes the platform, repository responsibilities, architecture, local SIT workflow, security boundaries, and deferred AWS/Sonar scope without publishing secrets or claiming production readiness.
- Public release remains blocked until the confirmed P1 security findings are addressed or explicitly accepted: service-level protection for Ledger mutations, customer/account object authorization, token audience isolation, and Redis authentication/network restriction. Visibility and branch protection remain separate owner-controlled settings changes.

### 2026-09-12 - Security hardening merged

- The public-release hardening wave is merged. Ledger mutations now require `ledger.internal`; customer and account self-service routes enforce ownership scopes; Auth issues audience and token-purpose claims; Payment, Transaction, MFA, and Gateway validate those claims; and SIT Redis uses an external Secret plus gateway-only network access.
- Merged implementation PRs: [ledger-service #26](https://github.com/digital-bank-java/ledger-service/pull/26), [customer-service #38](https://github.com/digital-bank-java/customer-service/pull/38), [account-service #47](https://github.com/digital-bank-java/account-service/pull/47), [auth-service #15](https://github.com/digital-bank-java/auth-service/pull/15), [config-repo #53](https://github.com/digital-bank-java/config-repo/pull/53), [config-repo #54](https://github.com/digital-bank-java/config-repo/pull/54), [infra-sit #46](https://github.com/digital-bank-java/infra-sit/pull/46), [api-gateway #32](https://github.com/digital-bank-java/api-gateway/pull/32), [payment-service #15](https://github.com/digital-bank-java/payment-service/pull/15), [transaction-service #34](https://github.com/digital-bank-java/transaction-service/pull/34), and [mfa-service #23](https://github.com/digital-bank-java/mfa-service/pull/23).
- The next step is merged-mainline local SIT rollout and verification. This must confirm the new `ledger.internal`, ownership, JWT audience/purpose, Redis Secret, and NetworkPolicy behavior before public visibility and branch protection are enabled.

### 2026-09-12 - Merged-mainline SIT rollout verification

- Rolled the merged application images into local `digital-bank-sit`; all application and infrastructure workloads reached `1/1 Running`.
- Verified API Gateway health (`200`) and fail-closed behavior for unauthenticated customer/account routes and centralized Swagger (`401`).
